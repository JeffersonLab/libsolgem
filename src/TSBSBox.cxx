...
using namespace NSBSBox;

TSBSBox::TSBSBox (Double_t r0, Double_t r1, Double_t phi0, Double_t dphi)
  : fR0R0(0),
    fR1R1(1),
    fPhi0(0),
    fTPhi0(0),
...
Int_t
NSBSBox::CmpAng (Double_t x, Double_t y, Double_t phi, Double_t tphi)
{
...
void
TSBSBox::SetGeometry (const Double_t r0,
			const Double_t r1,
			const Double_t phi0,
			const Double_t dphi)
{
...
void
TSBSBox::LabToWedge (Double_t& x, Double_t& y) const
{
  x -= (GetOrigin())[0];
  y -= (GetOrigin())[1];
  Double_t temp = x;
...
void
TSBSBox::WedgeToLab (Double_t& x, Double_t& y) const
{
  Double_t temp = x;
  x = fCLW * x + fSLW * y;
  y = -fSLW * temp + fCLW * y;
